...
{
  AssertExc(DIM == 1,  "this method can only be used for 1d.");
  AssertExc(DOW == 1,  "this method can only be used for 1d.");
  int i, j;
  std::ifstream is(s.c_str());
  is >> i;
...
std::istream& operator>>(std::istream& is, THIS& m)
{
  ...
  for (i = 0;i <= DIM;i ++) {
    ...
    for (k = 0;k < j;k ++) {
      is >> temp;
      l = temp.index();
      AssertExc(l < j, "geometry index error.");
      g[l] = temp;
    }
...
TemplateGeometry<DIM>& TemplateGeometry<DIM>::operator=(const TemplateGeometry<DIM>& t)
{
  ...
  void TemplateGeometry<DIM>::loadFunction()
  {
    ...
    AssertExc(symbol, volume_function_name.c_str());
...
filtering_istream& operator>>(filtering_istream& is, TemplateGeometry<DIM>& t)
{
  ...
  for (i = 0;i <= DIM;i ++) {
    ...
    for (k = 0;k < j;k ++) {
      ...
      m = v.size() - 1;
      for (l = 0;l < m;l ++)
	AssertExc(v[l] < v[l+1], "geometry vertices not sorted.");
      l = temp.index();
      AssertExc(l < j, "geometry index error.");
