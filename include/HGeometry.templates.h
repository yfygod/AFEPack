...
void THIS::checkIntegrity() const
{
  ...
  for (int i = 0;i < THIS::n_boundary;i ++) {
	...
	if (b->vertex[j] == vertex[k]) {
      ...
      AssertExc(k < (THIS::n_vertex), "internal error.");
    }
  }
  /**
...
{
  AssertExc (regular_mesh != NULL, "internal error.");
  int i, j, k, l, m, n;
...
std::ostream& operator<<(std::ostream& os, const HElement<DIM, DOW>& element)
{
  if (element.value == 1) {
    ...
    AssertExc(false, "internal error."); // what happeden? something must be wrong!
  }
  return os;
}
...
mesh0(&m0), mesh1(&m1)
{
  AssertExc(mesh0->geometry_tree == mesh1->geometry_tree, "internal error.");
}
...
THIS& THIS::operator++()
{
  if (iterator0.element == NULL && iterator1.element == NULL) {
    AssertExc(st == EQUAL, "internal error.");
  } else if (st == EQUAL) {
    ++ iterator0;
    ++ iterator1;
    if (iterator0.element == NULL || iterator1.element == NULL) {
      AssertExc (iterator0.element == NULL && iterator1.element == NULL, "internal error.");
      return *this;
    }
    ...
    if (iterator1.element == NULL) {
      AssertExc(next0.element == NULL, "internal error.");
      iterator0 = next0;
      st = EQUAL;
    ...
    if (iterator0.element == NULL) {
      AssertExc(next1.element == NULL, "internal error.");
      iterator1 = next1;
      st = EQUAL;
...
{
  AssertExc (DIM == 2, "internal error.");
}
