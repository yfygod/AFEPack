...
namespace boost {
  namespace serialization {
    ...
    template <class AR, class T> void
      serialize(AR& ar, 
                AFEPack::BinaryBuffer<T>& buf, 
                const u_int version) {
      ar & boost::serialization::base_object<typename AFEPack::BinaryBuffer<T>::_Base>(buf);;
    }
    ...
    template <class AR, int DIM, int DOW> void
      serialize(AR& ar, 
                AFEPack::HGeometryTree<DIM,DOW>& ht, 
                const u_int version) {
      ar & ht.rootElement();
      ar & ht.is_locked();
    }
    ...
    template <class AR, int DIM, int DOW> void
      load(AR& ar,
           AFEPack::HGeometryTree<DIM,DOW>& tree,
           const u_int version = 0) {
      u_int n_root_ele;
      ar & n_root_ele;
      for (u_int i = 0;i < n_root_ele;++ i) {
        AFEPack::HGeometry<DIM,DOW> * ele = new AFEPack::HGeometry<DIM,DOW>();
        ar & *ele;
    ...
    template <class AR, int DIM, int DOW> void
      ...
      typename AFEPack::HGeometryTree<DIM,DOW>::RootIterator
    ...
    template <class AR, int DIM, int DOW> void
      serialize(AR& ar, 
                AFEPack::IrregularMesh<DIM,DOW>& im, 
                const u_int version) {
      ar & im.rootElement();
      ar & &(im.geometryTree());
    }
    ...
    template <class AR, int DIM, int DOW> void
      ...
      ar & n_root_ele;
      for (u_int i = 0;i < n_root_ele;++ i) {
        AFEPack::HElement<DIM,DOW> * ele = new AFEPack::HElement<DIM,DOW>();
        ar & *ele;
    ...
    template <class AR, int DIM, int DOW> void
      save(AR& ar,
           ...
           const u_int version = 0) {
      u_int n_root_ele = end - start;
      typename AFEPack::IrregularMesh<DIM,DOW>::RootIterator
