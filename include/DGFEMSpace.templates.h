  ...
  for (i = TDIM1-1;i > 0;i --) {
    ...
    for (j1 = 0, l = 0;j1 < j;j1 ++) {
      ...
      for (k1 = 0;k1 < k;k1 ++) {
        ...
        if (l1 == l) {
          AssertExc(l < t_geo.n_geometry(i), "internal error.");
    ...
    for (j1 = 0;j1 < j;j1 ++) {
      ...
      for (k1 = 0;k1 < k;k1 ++) {
        ...
        AssertExc(l1 < i1, "the vertex is not belong to the element.");
      }
      sort(v.begin(), v.end());
      ...
      AssertExc(k < l,"no such boundary in the template geometry.");
    }
  }
};
...
{
  AssertExc(i >= 0 && i < 2, "internal error.");
  return *neigh[i];
};
