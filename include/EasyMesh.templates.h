...
void TriangleMesh<DOW>::readData(const std::string& filename)
{
  ...
  std::cerr << "\treading node data ..." << std::flush;
  ...
  std::cerr << "\treading side data ..." << std::flush;
  is.open((filename + ".s").c_str());
  is >> i;
  AssertExc(i == n_side, "in side file: side number error");
  ...
  std::cerr << "\treading element data ..." << std::flush;
  is.open((filename + ".e").c_str());
  is >> i;
  AssertExc(i == n_element,  "in element file: element number error");
  is >> i;
  AssertExc(i == n_node,  "in element file: node number error");
  is >> i;
  AssertExc(i == n_side,  "in element file: side number error");
  is.getline(text, 64);
...
void TriangleMesh<DOW>::writeData(const std::string& filename) const
{
  ...
  for (i = 0;i < n_element;i ++) {
	...
	AssertExc(the_side.vertex(1) == the_element.vertex((j+2)%3), "internal error.");
	side_neighbour[0][k] = i;
      }
      else if (the_side.vertex(0) == the_element.vertex((j+2)%3)) {
	AssertExc(the_side.vertex(1) == the_element.vertex((j+1)%3), "internal error.");
