...
{
  AssertExc (sp != NULL, "internal error.");
  geometry_index = g;
  template_element_index = t;
}
                           ...
                           int rank)
{
  ...
  for (;it_el < end_el;++ it_el) {
	...
	if (flag[i][l]) {
	  AssertExc(dof().n_geometry_dof[i][l] == m, "DOF number mismatch.");
	}
	else {
	  flag[i][l] = true;
  ...
  for (;it_el < end_el;++ it_el) {
	...
	if (!flag[i][l]) {
	  ...
	  for (n = 0;n < m;n ++) {
	    ...
	    AssertExc(i1 < m, "DOF data mismatch.");
	  }
	}
	else {
	  flag[i][l] = false;
								     ...
								     bool preserve_symmetry)
{
  ...
  AssertExc (A.n() == A.m(), "dimensional mismatch.");
  AssertExc (A.n() == f.size(), " dimensional mismatch.");
  AssertExc (A.n() == u.size(), " dimensional mismatch.");
  AssertExc (A.n() == n_dof, " dimensional mismatch.");
