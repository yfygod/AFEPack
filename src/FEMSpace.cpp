...
/**
  *  flag=0: piecewise linear solution
  *  flag=1: piecewise const solution
  */
        ...
        int flag) const
{
    ...
    /** 
     *  The twin triangle is divided into two triangles for visualization.
     */
    int n_data = 0;
    int n_element = mesh.n_geometry(2);
    for (int i = 0;i < n_element;i ++)
    {
        switch (mesh.geometry(2,i).n_vertex())
        {
        case 3:
            n_data += 1;
            break;
    ...
    n_element = n_data; // Total number of elements

    /**
     *  Transform piecewise const data into piecewise linear one
     */

    if (flag == 0) // piecewise const values
        n_data = n_node;

    FEMSpace<double,2>::ConstElementIterator the_element = fem_space.beginElement();
    FEMSpace<double,2>::ConstElementIterator end_element = fem_space.endElement();
    ...
    for (;the_element != end_element;the_element ++)
    {
        ...
        else if (flag == 1)
        {
            ...
            if (geo.n_vertex() == 4)
            {
                ...
                bc[0] /= 3; bc[1] /= 3;
                val[j ++] = value(bc, *the_element);// j equals to the index of element
            }
        }
    }
    ...
    os << "Zone N=" <<n_node << ", E=" << n_element <<", F=FEPOINT, ET=TRIANGLE" << std::endl;

    for (i = 0;i < n_node;i ++)
    {
        os << mesh.point(i) <<" "<< val[i] << "\n";
    }
    ...
    for (int j = 0;the_element != end_element;the_element ++)
    {
        ...
        {
        case 3:
            os << mesh.geometry(0, geo.vertex(0)).vertex(0)+1 << "\t"
            << mesh.geometry(0, geo.vertex(1)).vertex(0)+1 << "\t"
            << mesh.geometry(0, geo.vertex(2)).vertex(0)+1 << "\t\n";
            break;
        case 4:
            os << mesh.geometry(0, geo.vertex(0)).vertex(0)+1 << "\t"
            << mesh.geometry(0, geo.vertex(1)).vertex(0)+1 << "\t"
            ...
            os << mesh.geometry(0, geo.vertex(0)).vertex(0)+1 << "\t"
            << mesh.geometry(0, geo.vertex(2)).vertex(0)+1 << "\t"
            << mesh.geometry(0, geo.vertex(3)).vertex(0)+1 << "\t\n";
            break;
        default:
            Assert(false, ExcInternalError());
        }
...
template <>
void FEMFunction<double, 2>::writeTecplotData_Rectangle(const std::string& filename,
                                                        int flag) const
{
    std::ofstream os(filename.c_str());
    os.precision(12);
    ...
    for (int j = 0;the_element != end_element;the_element ++)
    {
        ...
        os << mesh.geometry(0, geo.vertex(0)).vertex(0)+1 << "\t"
           << mesh.geometry(0, geo.vertex(1)).vertex(0)+1 << "\t"
           << mesh.geometry(0, geo.vertex(2)).vertex(0)+1 << "\t"
           << mesh.geometry(0, geo.vertex(geo.n_vertex()-1)).vertex(0)+1 << "\t\n";
    }
    os.close();
}
        ...
        int flag) const
{
    ...
    std::vector<double> val(n_node, 0);
    int i, j, k;
    FEMSpace<double,3>::ConstElementIterator the_element = fem_space.beginElement();
    FEMSpace<double,3>::ConstElementIterator end_element = fem_space.endElement();
    ...
    int n_element = mesh.n_geometry(3);
    for (i = 0, j = 0;i < n_element;i ++)
    {
        switch (mesh.geometry(3,i).n_vertex())
        {
        case 4:
            j += 1;
            break;
    ...
    os << "Zone N=" <<n_node << ", E=" << j <<", F=FEPOINT, ET=TETRAHEDRON" << std::endl;
    ...
    for (i = 0;i < n_element;i ++)
    {
        ...
        {
        case 4:
            os << mesh.geometry(0, mesh.geometry(3,i).vertex(0)).vertex(0)+1<< "\t"
            << mesh.geometry(0, mesh.geometry(3,i).vertex(1)).vertex(0)+1 << "\t"
            << mesh.geometry(0, mesh.geometry(3,i).vertex(2)).vertex(0)+1 << "\t"
            << mesh.geometry(0, mesh.geometry(3,i).vertex(3)).vertex(0)+1 << "\t\n";
            break;
        case 5:
            os << mesh.geometry(0, mesh.geometry(3,i).vertex(0)).vertex(0)+1 << "\t"
            << mesh.geometry(0, mesh.geometry(3,i).vertex(1)).vertex(0)+1 << "\t"
            << mesh.geometry(0, mesh.geometry(3,i).vertex(2)).vertex(0)+1 << "\t"
            << mesh.geometry(0, mesh.geometry(3,i).vertex(4)).vertex(0)+1 << "\t\n";
            os << mesh.geometry(0, mesh.geometry(3,i).vertex(0)).vertex(0)+1 << "\t"
            << mesh.geometry(0, mesh.geometry(3,i).vertex(2)).vertex(0)+1 << "\t"
            << mesh.geometry(0, mesh.geometry(3,i).vertex(3)).vertex(0)+1 << "\t"
            << mesh.geometry(0, mesh.geometry(3,i).vertex(4)).vertex(0)+1 << "\t\n";
        ...
        case 7:
            os << mesh.geometry(0, mesh.geometry(3,i).vertex(0)).vertex(0)+1 << "\t"
            << mesh.geometry(0, mesh.geometry(3,i).vertex(1)).vertex(0)+1 << "\t"
            << mesh.geometry(0, mesh.geometry(3,i).vertex(6)).vertex(0)+1 << "\t"
            << mesh.geometry(0, mesh.geometry(3,i).vertex(5)).vertex(0)+1 << "\t\n";
            os << mesh.geometry(0, mesh.geometry(3,i).vertex(0)).vertex(0)+1<< "\t"
            << mesh.geometry(0, mesh.geometry(3,i).vertex(2)).vertex(0)+1<< "\t"
            << mesh.geometry(0, mesh.geometry(3,i).vertex(4)).vertex(0)+1 << "\t"
            << mesh.geometry(0, mesh.geometry(3,i).vertex(6)).vertex(0)+1 << "\t\n";
...
/**
 *  Cheney   April 25, 2014
 *	Solutions are saved to the file "filename". 
 *	Meshdata (points and triangles) are saved to the folder "dir"
 */

template <>
void FEMFunction<double, 2>::writeMatlabData(const std::string& filename,
																						 const std::string& dir) const
{
	// Point data
	std::string fname;
	fname=dir+"/points";
  std::ofstream osp(fname.c_str());
  osp.precision(12);
  osp.setf(std::ios::fixed, std::ios::floatfield);
  const FEMSpace<double,2>& fem_space = femSpace();
  const Mesh<2,2>& mesh = fem_space.mesh();
  int n_node = mesh.n_point();
  for (int i = 0;i < n_node;i ++) 
  ...
  for (int j = 0;the_element != end_element;++ the_element) 
	{
		  ...
		  switch (geo.n_vertex()) 
			{
				case 3:
				  ost << mesh.geometry(0, geo.vertex(0)).vertex(0)+1 << "\t"
			 << mesh.geometry(0, geo.vertex(1)).vertex(0)+1 << "\t"
			 << mesh.geometry(0, geo.vertex(2)).vertex(0)+1 << "\t1\n";
				  break;
				case 4:
				  ost << mesh.geometry(0, geo.vertex(0)).vertex(0)+1 << "\t"
			 << mesh.geometry(0, geo.vertex(1)).vertex(0)+1 << "\t"
			 << mesh.geometry(0, geo.vertex(2)).vertex(0)+1 << "\t1\n";
				  ost << mesh.geometry(0, geo.vertex(0)).vertex(0)+1 << "\t"
			 << mesh.geometry(0, geo.vertex(2)).vertex(0)+1 << "\t"
			 << mesh.geometry(0, geo.vertex(3)).vertex(0)+1<< "\t1\n";
				  break;
				default:
				  Assert(false, ExcInternalError());
    	}
