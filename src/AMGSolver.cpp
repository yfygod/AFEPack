...
#define ExcDimensionMismatch(x, y) "dimension mismatch."
#define ExcNotCompressed() "not compressed."
...
void AMGSolver::Project(const Matrix& M, 
			...
			Matrix *& PMPt)
{
  AssertExc(M.m() == M.n(), ExcDimensionMismatch(M.m(), M.n()));
  const SparsityPatternBase<>& spM = M.get_sparsity_pattern();
  ...
  for (u_int i = 0, n = 0;i < M.m();i ++) {
    ...
    AssertExc(flag[i] == -1, "internal error.");
    core_point[n] = i;
    core_at[i] = n; //! 
   ...
   * 
   */
  SparsityPatternBase<>& spP = *(new SparsityPatternBase<>(n_core_point, M.m(), row_length));
  SparsityPatternBase<> spPt = SparsityPatternBase<>(M.m(), n_core_point, col_length);
			    ...
			    Matrix *& Pt)
{
  ...
  SparsityPatternBase<>& spP = *new SparsityPatternBase<>(n_core_point, M.n(), spM.max_entries_per_row());
  SparsityPatternBase<>& spPt = *new SparsityPatternBase<>(M.n(), n_core_point, spM.max_entries_per_row());
  for (u_int i = 0;i < n_core_point;i ++) {
    const u_int& k = core_point[i];
    spP.add(i, k);
    spPt.add(k, i);
...
{
  const SparsityPatternBase<>& spP = P.get_sparsity_pattern();
  const SparsityPatternBase<>& spM = M.get_sparsity_pattern();
  const SparsityPatternBase<>& spPt = Pt.get_sparsity_pattern();
  const std::size_t * P_rowstart = spP.get_rowstart_indices();
  const u_int * P_colnums = spP.get_column_numbers();
  const std::size_t * M_rowstart = spM.get_rowstart_indices();
  const u_int * M_colnums = spM.get_column_numbers();
  ...
  SparsityPatternBase<>& spA = *(new SparsityPatternBase<>(P.m(), row_length));
  for (u_int i = 0;i < P.m();i ++) {
    for (u_int j = 0;j < row_length[i];j ++) {
...
void AMGSolver::reinit(const Matrix& M)
{
  ...
  const SparsityPatternBase<>& spM = M.get_sparsity_pattern();
  AssertExc(spM.is_compressed(), ExcNotCompressed());
  AssertExc(M.m() == M.n(), ExcDimensionMismatch(M.m(), M.n()));
  u_int k, m;
  k = m = M.m();
  ...
  while(m > int(n_most_project_order) || sparse_degree < n_most_project_sparse_degree) {
    ...
    if (k == m) {
      ...
      const SparsityPatternBase<>& spP = P->get_sparsity_pattern();
      delete P; delete (&spP);
      const SparsityPatternBase<>& spM = PMPt->get_sparsity_pattern();
...
void AMGSolver::lazyReinit(const Matrix& M)
{
  ...
  for (j = 0;j < n_project;j ++) {
    const SparsityPatternBase<>& spP = projected_matrix[j + 1]->get_sparsity_pattern();
...
void AMGSolver::solve(Vector<double>& x, 
		      ...
		      int mode) const
{
  AssertExc(is_initialized == true, "not initialized");
  AssertExc(projected_matrix[0]->m() == r.size(), 
         ExcDimensionMismatch(projected_matrix[0]->m(), r.size()));
  AssertExc(x.size() == r.size(), 
...
{	
  ...
  for (u_int i = 0;i < n_project;i ++) {
    const SparsityPatternBase<>& spP = project_matrix[i]->get_sparsity_pattern();
    delete project_matrix[i];
    delete (&spP);
    const SparsityPatternBase<>& spM = projected_matrix[i+1]->get_sparsity_pattern();
  ...
  if (project_matrix_r.size() > 0) {
    for (u_int i = 0;i < n_project;i ++) {
      const SparsityPatternBase<>& spP = project_matrix_r[i]->get_sparsity_pattern();
